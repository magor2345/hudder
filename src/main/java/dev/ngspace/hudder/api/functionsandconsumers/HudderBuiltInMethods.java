...
import dev.ngspace.hudder.uielements.*;
import dev.ngspace.hudder.uielements.EntityModel.EntityModelElement;
import dev.ngspace.hudder.uielements.EntityModel.EntityModelLookingAt;
...
public class HudderBuiltInMethods {private HudderBuiltInMethods() {}
	...
	public static void registerMethods(FunctionAndConsumerAPI api) {
		...
		api.registerConsumer(
				(IUIElementManager e, AHudCompiler<?> a, ObjectWrapper[] s) -> {
					try {
						String entityId = s[0].asString().trim();
						int x = s[1].asInt();
						int y = s[2].asInt();
						float bodyRot = s[3].asFloat();
						float xRot = s[4].asFloat();
						float yRot = s[5].asFloat();
						float xHitboxRot = s[6].asFloat();
						float zHitboxRot = s[7].asFloat();
						float size = (float) s[8].asDouble();
						...
						e.addUIElement(new EntityModelElement(id, x, y, bodyRot, xRot, yRot, xHitboxRot, zHitboxRot, size));

					} catch (CompileException ex) {
						throw new RuntimeException("Failed to parse drawEntity arguments", ex);
					}
				},
				"drawEntity",
				"entity"
		...
		api.registerConsumer(
				(IUIElementManager e, AHudCompiler<?> a, ObjectWrapper[] s) -> {
					try {
						int x = s[0].asInt();
						int y = s[1].asInt();
						float bodyRot = s[2].asFloat();
						float xRot = s[3].asFloat();
						float yRot = s[4].asFloat();
						float xHitboxRot = s[5].asFloat();
						float zHitboxRot = s[6].asFloat();
						float size = (float) s[7].asDouble();
						e.addUIElement(new EntityModelLookingAt(x, y, bodyRot, xRot, yRot, xHitboxRot, zHitboxRot, size));
					} catch (CompileException ex) {
						throw new RuntimeException("Failed to parse drawEntityLookingAt arguments", ex);
					}
				},
				"drawEntityLookingAt",
				"draw_entity_lookin_at"
		);
